...
Operation.prototype.compile = function () {
    ...
    case '===':
    case '!==':
        if (self.isNil(self.left) && self.right instanceof Identifier && !scopes.isDefined(self.right.value)) {
            va = self.right.compile();
            return 'typeof ' + va + ' ' + self.operator + " 'undefined' || " + va + ' ' + self.operator + ' null';
        }
        if (self.isNil(self.right) && self.left instanceof Identifier && !scopes.isDefined(self.left.value)) {
