var ARGUMENT, BOOL_RE, COMMENT_RE, FLOAT_RE, IDENTIFIER_RE, INTEGER_RE, KEYWORDS, KEYWORDS_RE, LINEFEED_RE, Lexer, NIL_RE, OPERATORS, OPERATORS_RE, PARENS, PARENS_RE, REGEXP, REGEXP_RE, SEMICOLON_RE, SKIP_LF_AFTER, SKIP_LF_BEFORE, STRING2_RE, STRING_RE, Token, WHITESPACE_RE, enhanced_array, escape, re, words;
escape = function (str) {
    return str.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
};
re = function (ary, bound) {
    var map;
...
Lexer.prototype.rewrite = function (tokens) {
    var self = this;
    var i, next_token, prev, rs, s, skip, state, token;
    i = 0;
    rs = enhanced_array();
    ...
    while (token = tokens[i]) {
        ...
        case 'or':
        case 'not':
            if ((rs.last() && (rs.last().name === '.' || rs.last().name === 'def')) || (next_token() && next_token().name === ':')) {
