...
// Idents a preformated AST (as nested arrays).
...
// FIXME: WHEN statements may have multiple expressions!
...
function parseObjectDeclaration(decl) {
  var left;
  switch (decl[1][0]) {
    // we skip parseExpression() because that would consider this identifier to be a variable, which it's not.
    case 'ident':
      left = decl[1][1];
    break;
    
    // NOTE: disabled until we check for javascript keywords.
//    case 'string':
//      left = decl[1][1];
//      if (left.match(/^('|")[a-zA-Z_][0-9a-zA-Z_]*\1$/)) {
//        eval("left = " + left + ";");
//      }
//    break;
    
    default:
      left = parseExpression(decl[1]);
  }
