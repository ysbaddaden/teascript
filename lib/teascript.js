...
function parseStatement(rs, expr) {
  ...
  case 'def':
    withScope(function () { // we scope to skip method arguments
      rs.push("function " + expr[1] + "(" + parseArgumentDeclarations(expr[2]) + ") {" );
      rs.push(parseBodyWithScope(expr[3]));
      rs.push("}");
    });
  break;
...
//      console.warn("WARNING: undefined identifier '" + expr[1] + "'.");
  ...
  case "invoke":
    rs.push(parseFunctionName(expr[1]) + "(" + parseDeclarationList(expr[2]) + ")");
  break;
...
function parseFunctionName(expr) {
  return (expr[0] == "ident") ? expr[1] : parseExpression(expr[1]);
}
